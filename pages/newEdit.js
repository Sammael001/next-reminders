...
export default function NewEdit() {
  ...
  // VV this causes a memory leak if we navigate away from newEdit before 3s is up?
  // useEffect(() => {
  //   if (showMsg) {
  //     setTimeout(() => {
  ...
  function onSubmitForm(evt){
    ...
    let newReminderMap = saved.map(remObj => {
      if (remObj.caseID === idToEdit) {
        return myNewCaseObj; // ..and overwrite that obj with the formVal obj
      } else {
        return remObj;
      }
    });
    let sorted = sortDates(newReminderMap); // sort newReminderMap by date
  ...
  function copyToClipboard(evt){
    ...
    // console.log(formVal[evt.target.id]);
...
//     <h1 className={styles.title}>Delete this reminder?</h1>
//     <div className={styles.butnBox}>
//       <button className={styles.bluButn} onClick={() => setShowDelete(false)}>Nevermind</button>
//       <button className={styles.redButn} onClick={deleteRem}>Yes, Delete</button>
//     </div>
//   </div>
// }
//
